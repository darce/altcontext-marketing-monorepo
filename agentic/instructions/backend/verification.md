...
- Version-gated SQL features validated against the actual PG major version in use (checklist #16)
- Prisma schema defaults vs DB defaults verified with `prisma migrate diff` after default-related migrations (checklist #17)
- "Remaining usage" grep claims explicitly scoped to runtime (`src/`) vs tests/dev tooling (`test/`, scripts) (checklist #18)
- Schema resolution source-of-truth is shared across runtime/test helpers/SQL builders; conflicting schema inputs fail fast (checklist #19)
- Destructive test workflows target an isolated test DB/schema and cannot collide with the default dev target (checklist #20)
- SECURITY DEFINER functions enforce least privilege: no implicit PUBLIC execute and pinned trusted search path (checklist #21)
