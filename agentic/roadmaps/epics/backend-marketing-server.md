...
- Enable PostgreSQL Row-Level Security (RLS) enforced via `SET LOCAL app.current_tenant_id`.
- Replace `ADMIN_API_KEY` env var with per-tenant API key resolution.
- Add dashboard authentication (bcrypt password + encrypted cookie sessions).
