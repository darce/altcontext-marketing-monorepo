...
- [x] All SQL queries in services are exercised by at least one integration test. (Post-MT-2: all major service paths covered by rls-isolation + route tests.)
- [x] Honeypot path tested: event with honeypot field → 202 response, zero rows in DB. ✅
- [x] Delete-by-email tested: PII in `form_submissions.payload` is scrubbed or cascade-deleted. (leads-delete.test.ts)
- [x] Transaction rollback tested: error mid-transaction → no partial writes. (db.test.ts)
- [x] Task checklist items in this file and parent tasks (0.4, 0.4.1) updated to reflect completed work.
