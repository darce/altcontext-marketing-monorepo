...
- [ ] Deploy migration to production (Fly.io).
- [ ] Verify `app_user` role exists in production database.
- [ ] Verify RLS is enabled: `SELECT relname, relrowsecurity FROM pg_class WHERE relrowsecurity = true`.
- [ ] Verify ingest beacons continue working (route uses `withTenant` → RLS-enforced).
- [ ] Verify admin endpoints continue working.
- [ ] Verify rollup scripts run successfully with `RESET ROLE`.
- [ ] **Gate**: `fly logs` shows no RLS-related errors.
- [ ] **Gate**: `make -C backend audit && make -C backend test` — full pass.

## Stretch Goals

- [ ] `FORCE ROW LEVEL SECURITY` on tables to enforce RLS even for the table owner (extra safety).
- [ ] `pg_stat_user_tables` monitoring: confirm index scans (not seq scans) on `tenant_id` for RLS filter.
- [ ] Per-tenant connection pooling (PgBouncer) — deferred until horizontal scaling needed.
