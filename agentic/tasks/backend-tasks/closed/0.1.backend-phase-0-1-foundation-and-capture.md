# Backend Phase 0-1: Foundations and Capture — ✅ CLOSED

> **Status**: All phases complete. Implementation verified in [0.1.1 code review](./0.1.1.phase-0-1-code-review.md). Event ingestion, lead capture, consent management, identity linking, and compliance workflows all operational.
...
- [x] Add backend TS runtime scaffold (`src/`) with Fastify entrypoint.
- [x] Add env schema validation and startup failure on invalid config.
- [x] Add Prisma setup and initial schema for MVP entities.
- [ ] Run and commit first Prisma migration.
- [x] Add `GET /v1/healthz` endpoint.
- [x] Add structured logging with request-id correlation and PII redaction (no raw IPs/emails in log output).
- [x] Add Fly-compatible start command and listen host/port behavior (`0.0.0.0`).

## Phase 1: Capture + Identity

- [x] Implement `POST /v1/events` with schema validation and response contract.
- [x] Implement `POST /v1/leads/capture` with email normalization and upsert logic.
- [x] Persist `visitors`, `sessions`, `events`, `leads`, `lead_identities`, `form_submissions`.
- [x] Add suggested indexes from roadmap §5 (`events(visitor_id, timestamp)`, `leads(email_normalized)`, etc.).
- [x] Define session creation and timeout strategy (e.g. 30-min inactivity window, new session on new UTM params).
- [x] Enforce primary identity link via `anonId + email` on submit.
- [x] Add optional heuristic link path (`ip_hash + ua_hash + time window`) with lower confidence.
- [x] Implement `consent_status` and `consent_events` audit logging.
- [x] Implement `POST /v1/leads/unsubscribe` to set `consent_status=withdrawn`.
- [x] Add rate limiting and honeypot/bot mitigation on write endpoints.
- [x] Ensure no route blocks frontend render path; endpoints must be async and non-blocking.

## Compliance (Phase 1)

- [x] Add delete-by-email endpoint or admin command for PIPEDA erasure requests.
- [x] Stub automated retention purge job (raw events > 90 days, raw IP > 7 days) — can be a manual script initially.
- [x] Document privacy contact in project README.
