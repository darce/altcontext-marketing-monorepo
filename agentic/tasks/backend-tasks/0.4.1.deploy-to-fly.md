...
- `infra/fly/fly.toml` — 256MB VM, Canadian region, no `release_command` ✅
...
1. Provision Fly app + Managed Postgres in Canadian region (`yyz`).
...
| `backend/infra/fly/fly.toml` | Fly.io deployment config |
...
- [ ] **Gate**: Health check green from public internet.
...
- [ ] A lead capture creates `leads` + `lead_identities` + `form_submissions` rows.
