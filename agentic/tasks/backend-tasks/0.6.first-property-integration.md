...
| Gap | File | Impact |
|-----|------|--------|
| No UTM extraction | `static/src/assets/telemetry.ts` | UTM params from ad campaigns not captured |
| `BACKEND_URL` not set for production | GitHub Pages deploy workflow | Beacons target wrong URL or fail silently |
| No end-to-end verification | — | No proof that live beacons reach production DB |
...
- [ ] Add `getUtmParams()` to `telemetry.ts` — parse `utm_*` from `location.search`.
- [ ] Include UTM params in all beacon payloads (page_view, engagement, form_submit).
- [ ] **Gate**: `make -C static build` succeeds.

## Phase 1: Build & Deploy Config

- [ ] Set `BACKEND_URL=https://marketing-altcontext.fly.dev` in GitHub Pages build config (`.github/workflows/pages.yml`).
- [ ] Verify `BACKEND_URL` is injected into `telemetry.js` output.
- [ ] **Gate**: `make -C backend audit` — pass.
- [ ] **Gate**: `make -C backend test` — pass.

## Phase 2: Deploy & End-to-End Verification

- [ ] Set `BACKEND_URL=https://marketing-altcontext.fly.dev` in GitHub Pages build config.
- [ ] Deploy static site to GitHub Pages.
- [ ] Verify `page_view` beacon appears in production `events` table.
- [ ] Verify `engagement` beacon fires on tab switch / page close.
- [ ] Verify email form submission creates `leads` + `lead_identities` + `form_submissions` rows.
- [ ] Verify CWV metrics (LCP, CLS, INP) appear in event `props`.
- [ ] Verify UTM params appear in event `utm_source` / `utm_medium` / `utm_campaign` columns.
- [ ] Verify honeypot submission is silently discarded (202, no row).
- [ ] **Gate**: `fly logs` shows structured ingest events from the static site.

## Phase 3: Cleanup

- [ ] Close `static-tasks/email-capture-and-telemetry.md` (add CLOSED header, check completed phases).
- [ ] Update ROADMAP.md Phase 5 checklist items.
- [ ] **Gate**: `make -C backend audit && make -C backend test` — full pass.
...
- [ ] At least one `page_view` event from the live static site exists in the production `events` table.
- [ ] At least one lead capture from the live email form exists in the production `leads` table.
- [ ] No backend endpoint exposed in HTML source (`action="#"`).
- [ ] UTM params from URL query string appear in the production `events` table.
- [ ] `sendBeacon` fires reliably on page unload (engagement event stored).
