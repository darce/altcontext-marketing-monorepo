...
- **Deploy is live but incomplete**: The Fly app exists with a Postgres cluster provisioned and secrets set, but the schema migration has not been applied and the deployed image is stale (pre-Prisma-removal). Migration and redeploy come first.
...
- Fly app provisioned and deployed — container running in `yyz` region ✅
...
- **Missing**: Initial migration not applied to production DB, no smoke-test verification, no rollup job scheduled.
...
| `backend/infra/fly/fly.toml` | Fly.io deployment config (256MB, `yyz`) |
...
- [x] `fly.toml` committed with correct config (`infra/fly/fly.toml`).
...
- [ ] Run `fly proxy 15432:5432 -a <pg-app>` in a background terminal.
- [ ] Run `DATABASE_URL=postgres://postgres:<pw>@localhost:15432/<db> npx prisma migrate deploy` to apply initial migration.
- [ ] Verify migration applied: connect via proxy and check `SELECT * FROM _prisma_migrations`.
- [ ] Run `make -C backend fly-deploy` to redeploy with active database connection.
- [ ] Verify `fly status` shows 1 machine running.
- [ ] Verify `fly checks list` shows health check passing.
- [ ] `curl https://<app>.fly.dev/v1/healthz` returns `{ "ok": true }`.
- [ ] Check `fly logs` for clean startup (no errors, no Prisma references).
- [ ] **Gate**: Health check green from public internet with database connected.

## Phase 2: Production Smoke Tests

- [ ] `curl -X POST https://<app>.fly.dev/v1/events -H 'Content-Type: application/json' -d '<valid-payload>'` → 202.
- [ ] `curl -X POST https://<app>.fly.dev/v1/leads/capture -H 'Content-Type: application/json' -d '<valid-payload>'` → 200.
- [ ] `curl -H 'x-admin-key: <key>' https://<app>.fly.dev/v1/metrics/summary?window=7` → 200.
- [ ] CORS preflight from GitHub Pages domain → correct `Access-Control-Allow-Origin`.
- [ ] Rate limit: exceed 120 events/min → 429.
- [ ] Honeypot: submit with honeypot field → 202, verify no row in DB.
- [ ] Auth: `POST /v1/leads/delete` without admin key → 401.
- [ ] **Gate**: All smoke tests pass.

## Phase 3: Test Coverage — Phase 1 Edge Cases

- [ ] `test/events.test.ts` — Rate limit exceeded → 429.
- [ ] `test/leads.test.ts` — Invalid email → 400.
- [ ] `test/leads-delete.test.ts` — Form submission payload scrubbed/deleted after delete.
- [ ] `test/leads-delete.test.ts` — Nonexistent email → 404 or appropriate response.
...
- [ ] Run rollup job manually: `fly ssh console -C "node dist/scripts/rollups-run.js"` (or via proxy).
- [ ] Configure daily rollup execution (GitHub Action cron or Fly Machine schedule).
- [ ] Test `retention:purge` script (dry-run first if supported).
- [ ] Verify Fly dashboard shows Pino structured JSON logs.
- [ ] Document production URL and admin key location in team secrets manager.
- [ ] **Gate**: Rollup job produces non-zero data for at least one day.
...
- [ ] `https://<app>.fly.dev/v1/healthz` returns `{ "ok": true }` from the public internet with database connected.
- [ ] An event beacon payload is stored in the production database.
- [ ] A lead capture creates `leads` + `lead_identities` + `form_submissions` rows in production.
...
- [ ] `make -C backend test` passes with all Phase 3–5 tests green.
