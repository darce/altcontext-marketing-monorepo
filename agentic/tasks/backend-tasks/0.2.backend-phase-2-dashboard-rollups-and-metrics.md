...
- **Ingest Reliability**: Delivery quality indicators (success ratio, p95 latency, ingest failures).
...
- No rollup tables or materialized read model for daily KPI reporting.
- `GET /v1/metrics/summary` not implemented.
- No backfill/recompute CLI command for metrics windows.
- Admin auth helper (`hasValidAdminKey`) is private to `routes/leads.ts` — needs extraction for reuse.
- Acceptance checklist items for dashboard summaries and latency/error budgets remain open.
...
| Ingest p95 latency | `DailyIngestRollup` | Stored directly from ingestion timing samples |
...
export const assertAdminRequest = (request, reply) => {
  if (!env.ADMIN_API_KEY) {
    ...
    return reply.code(503).send({ ok: false, error: "admin_auth_not_configured" });
  }
...
```jsonc
{
  ...
  "metrics": {
    ...
    "trafficSourceMix": {
      "direct": 0.45, "organic_search": 0.25, "social": 0.15,
      "referral": 0.10, "paid_search": 0.03, "email": 0.02
    }
  },
  "ingest": {
    "successPct": 0.997,
  ...
  "trend": [
    { "day": "2026-02-07", "uniqueVisitors": 190, "pageViews": 780, "newLeads": 4 }
...
| `agentic/instructions/07-language-standards.md` | Shared TS standards (no redundant casts, no serial independent awaits). |
...
- [ ] Extract `hasValidAdminKey` + `assertAdminRequest` to `lib/admin-auth.ts`; update `routes/leads.ts` to import.
- [ ] Fix G3: backfill previous session `endedAt` in visitor service when new session starts on inactivity timeout.
- [ ] Fix G4: extend `purge-retention.ts` to delete orphaned sessions (no events) and visitors (no sessions).
- [ ] Add `DailyMetricRollup` and `DailyIngestRollup` models to Prisma schema.
- [ ] Create and apply Phase 2 migration with indexes for date-window summaries.
- [ ] Add `ROLLUP_BATCH_DAYS` and `ROLLUP_DEFAULT_PROPERTY_ID` to env config.
- [ ] Add script stubs and Make targets for `rollups-run`, `rollups-backfill`, `rollups-status`.

## Phase 1: Daily Rollup Generation

- [ ] Implement daily KPI derivation from `web_traffic_logs`, `events`, `sessions`, `leads`, and `form_submissions`.
- [ ] Compute all 7 roadmap §8 MVP traffic/funnel metrics per UTC day per property.
- [ ] Compute ingest reliability aggregates (success %, rejection count, p95 latency).
- [ ] Store traffic source breakdown and top landing paths as JSON columns.
- [ ] Ensure rollup upsert is idempotent — re-running for the same day+property overwrites with fresh values.
- [ ] Add property-aware aggregation; default to `"default"` for single-property installs.

## Phase 2: Metrics Summary API

- [ ] Add `schemas/metrics.ts` with Zod query validation for `from`, `to`, `propertyId`, optional `compareTo`.
- [ ] Implement `GET /v1/metrics/summary` reusing `assertAdminRequest` from `lib/admin-auth.ts`.
- [ ] Return all MVP metrics with `value` and `delta` (null when `compareTo` absent).
- [ ] Return daily `trend` array for sparkline/chart rendering.
- [ ] Return `trafficSourceMix` as proportional breakdown.
- [ ] Include `freshness` metadata (`rolledUpThrough`, `lagDays`, `generatedAt`).
- [ ] Include `ingest` reliability summary.
- [ ] Register `metricsRoutes` in `app.ts`.

## Phase 3: Tests

- [ ] Unit tests for metric formulas and edge cases (empty windows, single-day, timezone boundaries).
- [ ] Integration tests for endpoint auth (missing key, wrong key, valid key, unconfigured key).
- [ ] Integration tests for response contract shape stability.
- [ ] Database tests for rollup upsert idempotency and backfill correctness across day ranges.
- [ ] Performance smoke check for 30-day summary latency on local seeded dataset.

## Stretch Goals

- [ ] Add materialized-view option for heavy aggregations.
- [ ] Add lightweight in-memory caching for hot summary windows.
- [ ] Add discrepancy monitor comparing raw-event counts vs rollup totals.
- [ ] Add event dedup index to `events` table (code review G5) to improve rollup accuracy.
- [ ] Fix B3: pass upsert result directly in `captureLead` instead of re-reading consent status.
- [ ] Fix A3: cache HMAC key material to avoid per-call `createHmac` overhead.
- [ ] Batch heuristic link creation with `Promise.all` (code review E1).

## Success Criteria

- [ ] `GET /v1/metrics/summary` returns dashboard-ready MVP metrics for 7/30/90-day windows.
- [ ] Re-running rollups for the same date range yields deterministic, identical values.
- [ ] Metrics endpoint is protected by `ADMIN_API_KEY` and not publicly accessible.
- [ ] `compareTo` windows produce correct period-over-period `delta` values.
- [ ] Freshness metadata accurately reflects rollup lag.
- [ ] All code review prerequisites (G3, G4, auth extraction) resolved before rollup generation.
- [ ] Roadmap acceptance checklist item "Dashboard summary endpoint returns MVP metrics" can be marked complete.
